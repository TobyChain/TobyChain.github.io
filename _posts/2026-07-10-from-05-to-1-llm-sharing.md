...
<style>
  .post-content, .post-tail-wrapper {
    --llm-accent: #5645d4;
    --llm-surface: #fafaf9;
    --llm-hairline: #e5e3df;
    --llm-ink: #37352f;
  ...
  .post-content h2 {
    margin-top: 3rem;
    padding-top: 1rem;
    border-top: 1px solid var(--llm-hairline);
  }
  ...
  .post-content img {
    ...
    margin: 1.5rem auto;
    border-radius: 16px;
    border: 1px solid var(--llm-hairline);
    background: #fff;
  }
...
$y = w\_1 \cdot x\_1 + w\_2 \cdot x\_2 + b$

其中 $x\_1$ 是面积、$x\_2$ 是地铁距离，$w$ 是权重，$b$ 是偏置。训练的过程就是不断调整 $w$ 和 $b$，让模型的预测值和真实值之间的误差（线性回归通常用均方误差 MSE）最小化。这个过程用到的核心算法叫**梯度下降**，你站在山坡上，每次往最陡的方向迈一步，最终就能走到山谷（误差最小的地方）\[1\]。
...
虽然名字叫"回归"，但逻辑回归实际上是一个**分类模型**。它画出来的决策边界仍然是一条直线（或一个超平面），所以本质上它还是"线性模型家族"的一员 \[2\]。
...
$\text{Layer 1: } h = \sigma(W\_1 x + b\_1) \\ \text{Layer 2: } y = \sigma(W\_2 h + b\_2)$
...
1.  **ReLU 激活函数（2010 年）**：Sigmoid 和 Tanh 在层数多了之后会出现"梯度消失"问题，梯度连乘后趋近于零，深层的参数学不动了。ReLU（$f(x) = \max(0, x)$）简洁而优雅地缓解了这个问题，让训练几十上百层的网络成为可能 \[5\]。
...
简单来说，对于句子里的每个词，Self-Attention 会计算它和句子里所有其他词的"相关度"（通过 $Q$ 和 $K$ 的点积），然后按照相关度加权汇总所有词的信息（$V$），形成这个词的"上下文感知表示" \[7\]。
...
$\text{Masked Attention}: \quad \text{score}\_{i,j} = \begin{cases} Q\_i \cdot K\_j^T, & j \leq i \\ -\infty, & j > i \end{cases}$
...
2.  **规模效应（Scaling Laws）**。2020 年，OpenAI 和 DeepMind 的研究表明：模型性能与参数量、数据量、计算量之间存在可预测的幂律关系 \[11\]。Decoder-Only 架构结构简单、容易并行，特别适合"堆规模"。
...
$c\_t = W\_{DKV} h\_t \quad \text{(压缩：隐藏维度 → 低维潜在向量)} \\ K\_t = W\_{UK} c\_t, \quad V\_t = W\_{UV} c\_t \quad \text{(解压：低维 → 完整 K/V)}$

这使得 KV Cache 的大小从 $O(n \cdot h \cdot d)$ 降低到 $O(n \cdot c)$，其中 $c \ll h \cdot d$。在 DeepSeek-V3 中，这个压缩比大约能让 KV Cache 缩小 **5-13 倍**，对长文本推理至关重要 \[12\]。
...
**V3-0324 到 V3.1（2025年）**：V3.1 将 V3 线和 R1 线合并为一个 671B 的混合模型，核心创新是**可以在"快速回答"和"深度推理"之间切换**，只需修改 Chat Template 就能控制模型是直接回答还是先推理再回答 \[12\]。此外还引入了 **CoT 压缩训练**，让模型在推理时减少冗余的"思考 Token"。
...
传统 Transformer 用一条"残差流"在层间传递信息。V4 把它替换为**多条并行的残差流**，并施加双随机数学约束来保证信息在各流之间的稳定重分配。这让信息在超深网络中的流动更加高效和稳定 \[15\]。
...
如果纯靠人肉一篇篇读、一个个仓库翻，一天下来能看的东西很有限。我的策略是**用AI帮AI做第一轮筛选和预处理**，把"搜索-筛选-阅读"这条链路里最费时间的环节交给工具，自己只负责最后的"判断和吸收"。目前主要依赖两个自建工具：**Ascan（每日科技日报 Agent）** 和 **Learn-art Skill（深度学习报告生成器）**。每个工作日的技术摘要会同步到[钉钉知识库《技术摘要（持续更新）》](https://alidocs.dingtalk.com/i/nodes/o14dA3GK8gQlkoYwcKYo6jPLV9ekBD76?utm_scene=team_space)中。
...
目前我在 `.env` 里配置了 6 个核心挖掘方向，每个方向有独立的关键词权重和方向系数：

| 方向 | 方向名 | 权重 | 举例关键词 |
| --- | --- | --- | --- |
| DIGITAL\_WORKER | 数字员工 / 虚拟人 | 1.8 | digital avatar, talking head, NeRF |
| PRODUCT\_RECOMMEND | 商品推荐 | 1.5 | recommendation system, click-through rate |
| PRODUCT\_INSPECTION | 商品巡检 / 合规检测 | 1.3 | anomaly detection, defect detection |
| ECOM\_SERVICE | 电商客服 / 对话系统 | 1.4 | dialogue system, customer service |
| AGENTIC\_FRAMEWORK | 智能体框架 / MCP | 2.0 | llm-agent, tool calling, MCP, multi-agent |
| INTENT\_UNDERSTAND | 意图理解 / 用户行为 | 1.6 | intent classification, user behavior |

arXiv 覆盖 `cs.AI`、`cs.CL`、`cs.CV`、`cs.IR`、`cs.MA` 五个分类，每天最多处理 500 篇论文；GitHub 追踪 15 个 topic 方向的新兴项目，筛选出 Top 20 送入 LLM 深度分析。

**几个我觉得比较重要的工程细节**：

1.  **定时自主运行**：通过 macOS launchd 实现每个工作日 08:30 自动运行，周末自动跳过（arXiv 周末不发布），电脑休眠唤醒后会自动补跑一次。整套调度逻辑写在 `scripts/run.sh` 里，带幂等保护和失败重试（间隔 10 分钟重试一次），基本上装完就不用管了。

2.  **LLM 并发加速**：论文翻译和仓库分析都走 IdeaLab 的 Qwen3.6-Plus-DogFooding，用 `Semaphore(15)` + `ThreadPoolExecutor(15)` 做 15 路并行，实测 4 篇论文的分析从 4 分钟压缩到 1 分钟。

3.  **双格式输出**：同时生成 HTML（Notion 风格排版，本地浏览器直接看）和 Markdown（自动上传到钉钉知识库），HTML 自己看，MD 同步给团队。

4.  **评分系统可解释**：每篇论文最终会得到"极度推荐 / 很推荐 / 推荐 / 一般推荐"的评级，评分逻辑完全透明，核心词命中 +30、次要词 +8、顶会机构 +15，≥30 分才进日报，最多 15 篇。你能清楚看到每篇论文为什么被推荐、在哪个方向得分最高。
...
Learn-art 是我自建的 Claude Code Skill，它能把任意 **GitHub 仓库**或**论文**（arXiv 链接、PDF、Markdown）转换成一篇**单文件、离线可读的中文交互式 HTML 学习报告**。不是简单的摘要或翻译，而是一份按照固定 12 章节结构深入拆解的学习材料：
...
输出物是一个单文件 HTML，CSS/JS 全部内嵌，左侧 TOC 侧边栏滚动联动高亮，支持 Mermaid 图表渲染和代码高亮，断网也能打开阅读。设计上走的是"温暖的纸感"风格，米白底色配暖橘强调色，长时间阅读不累眼。
...
下面是两则示例，分别是针对 Anthropic 的"When AI Builds Itself"官方文档和 Genkit 仓库生成的学习报告：

[请至钉钉文档查看附件《anthropic-recursive-self-improvement.html》。](https://alidocs.dingtalk.com/i/nodes/AR4GpnMqJzKpOoklFa9oeYGD8Ke0xjE3?iframeQuery=anchorId%3DX02mq5114zhx1luhdev8re)

[请至钉钉文档查看附件《genkit.html》。](https://alidocs.dingtalk.com/i/nodes/AR4GpnMqJzKpOoklFa9oeYGD8Ke0xjE3?iframeQuery=anchorId%3DX02mq51151n69l84389qjj)
...
\[1\] 吴恩达，_Machine Learning_，Coursera/Stanford，2022. https://kyonhuang.top/Andrew-Ng-Machine-Learning-notes/#/week1

\[2\] 吴恩达，_Deep Learning Specialization - Neural Networks and Deep Learning_，Coursera，2017. https://kyonhuang.top/Andrew-Ng-Deep-Learning-notes/#/Neural\_Networks\_and\_Deep\_Learning/神经网络基础

\[3\] Minsky, M. & Papert, S., _Perceptrons: An Introduction to Computational Geometry_, MIT Press, 1969.

\[4\] Rumelhart, D., Hinton, G. & Williams, R., "Learning representations by back-propagating errors", _Nature_, 1986.

\[5\] Nair, V. & Hinton, G., "Rectified Linear Units Improve Restricted Boltzmann Machines", _ICML_, 2010.

\[6\] Datawhale, _DIY-LLM: 分词器原理与 BPE 实现_. https://datawhalechina.github.io/diy-llm/

\[7\] Vaswani, A. et al., "Attention Is All You Need", _NeurIPS_, 2017. https://arxiv.org/abs/1706.03762

\[8\] Devlin, J. et al., "BERT: Pre-training of Deep Bidirectional Transformers for Language Understanding", _NAACL_, 2019. https://arxiv.org/abs/1810.04805

\[9\] Radford, A. et al., "Improving Language Understanding by Generative Pre-Training", OpenAI, 2018.

\[10\] Brown, T. et al., "Language Models are Few-Shot Learners", _NeurIPS_, 2020. https://arxiv.org/abs/2005.14165
...
\[16\] 阿里云开发者社区, "分词器、Encoder、Decoder 学习笔记". https://developer.aliyun.com/article/1651850
